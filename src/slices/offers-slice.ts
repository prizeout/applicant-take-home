...
export interface OffersState {
    ...
    selected: SelectedPrizeoutOffers;
}
...
export const offersSlice = createSlice({
    ...
    name: 'offers',
    reducers: {
        toggleOffer(state, action: PayloadAction<PrizeoutOffer>) {
            const offer = action.payload;
            const firstGiftCard = offer.giftcard_list[0];
            const { checkout_value_id } = firstGiftCard;
            state.selected[checkout_value_id] = !state.selected[checkout_value_id] || false;
        },
    },
});

export const { toggleOffer } = offersSlice.actions;

export const selectOffers = ({ offers }: RootState): PrizeoutOffers => offers.offers;
export const selectActiveOfferIds = ({ offers }: RootState): SelectedPrizeoutOffers => offers.selected;
